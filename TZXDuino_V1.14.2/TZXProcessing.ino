...
void TZXPlay(char *filename) {
  ...
  if(pinState==LOW) {
        outputWriteLow(); // LowWrite(PORTB, PORTB1);
        }
        else
        {
          outputWriteHigh(); // HighWrite(PORTB, PORTB1); 
        }
...
void TZXSetup() {
    ...
    outputWriteLow(); // LowWrite(PORTB, PORTB1);             //Start output LOW
    isStopped=true;
    pinState=LOW;
...
void wave() {
  ...
  if(isStopped==0 && workingPeriod >= 1)
  {
      ...
      if (bitRead(workingPeriod, 14)== 0) {
        //pinState = !pinState;
        if (pinState == LOW)     outputWriteLow(); // LowWrite(PORTB, PORTB1);    
        else  outputWriteHigh(); // HighWrite(PORTB, PORTB1);
       } else {
        if (bitRead(workingPeriod, 13) == 0)     outputWriteLow(); // LowWrite(PORTB, PORTB1);    
        else  {
          outputWriteHigh(); // HighWrite(PORTB, PORTB1); 
